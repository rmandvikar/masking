...
namespace rm.Masking
{
	...
	/// 123456 => ******
	/// </remarks>
	public class HideMinXShowMaxYCharsPreserveLengthMask : IMaskValue
	{
		private readonly char maskCharacter;
		private readonly int minCharactersToMask;
		private readonly int maxCharactersToShow;
		...
		public HideMinXShowMaxYCharsPreserveLengthMask(
			...
			int maxCharactersToShow = DefaultMaxCharactersToShow)
		{
			...
			this.maxCharactersToShow = maxCharactersToShow;
		...
		public string Mask(string s)
		{
			...
			var showCharacters = Math.Min(maxCharactersToShow, Math.Max(0, s.Length - minCharactersToMask));
